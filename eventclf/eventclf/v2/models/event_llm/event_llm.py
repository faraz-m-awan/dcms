...
import re
import numpy as np
from prompts import EVENT_ATTENTENCE_TEMPLATE, ABOUT_EVENT_TEMPLATE
...
class EventLLM():
    def __init__(self,model:LLMModels=LLMModels.llama_3_2_1b_instruct,batch_size:int=10,device:Union[str,torch.device]="cpu"):
    ...
    def _format_prompt(self,prompt_template:str,prompt_inputs:Dict[str],posts:List[str])->List[str]:
        ...
        return prompt_list
    def predict(self,prompt_template:str,prompt_inputs:Dict[str],posts:List[str],temperature:float=0.1,max_new_tokens:int=1)->np.array:
        ...
        """
        prompt_list = self._format_prompt(prompt_template=prompt_template,prompt_inputs=prompt_inputs,posts=posts)
        output = self._pipe(prompt_list,max_new_tokens=max_new_tokens,temperature = temperature,eos_token_id=self._terminators)
        cleaned_output = []
