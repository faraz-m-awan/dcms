...
def ngrams(items, n, prefix):
    return [
        prefix + "_".join(items[start : start + n])
        for start in range(0, len(items) - n + 1)
    ]
...
def rich_analyzer_textual(doc):
    ...
    output = map(lambda x: x if not represents_float(x) else "numberfloat", output)
    output = map(
        lambda x: x
        if not is_time_format(str(x).replace("pm", "").replace("am", ""))
        else "hourdate",
        output,
    )
    ...
    output = map(lambda x: "not" if str(x) in ["not", "n't", "no"] else x, output)
    ...
    output = [
        x
        for x in output
        if len(x) > 1 and not x in ["#festivalname", "@festivalname", "festivalname"]
    ]
...
def rich_analyzer_w2v(doc):
    ...
    output = [x if len(soore.findall(x)) == 0 else "so" for x in output]
    output = [x if len(hashre.findall(x)) == 0 else x.replace("#", "") for x in output]
    output = [
        x if len(mentionre.findall(x)) == 0 else x.replace("@", "") for x in output
    ]
