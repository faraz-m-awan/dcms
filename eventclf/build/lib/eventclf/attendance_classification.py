...
from eventclf.feature_extraction import (
    adapt_features,
    get_group_of_features,
...
hash_tags = ["creamfields2016", "creamfields", "creamfield"] + [
    "vfestival",
    "V21st",
    "vfest2016",
    "vfest",
...
def get_dic_models(allalg=True):
    ...
    if allalg:
        ...
        dic_models["RandomForestClassifier_Comb_w2vGN_bow"] = RandomForestClassifier(
            random_state=random_state
        )
        dic_models[
            "GradientBoostingClassifier_Comb_w2vGN_bow"
        ] = GradientBoostingClassifier(random_state=random_state, n_estimators=300)
    else:
        dic_models[
            "GradientBoostingClassifier_Comb_w2vGN_bow_100_3_None"
...
def get_models_evaluation(model, idx_event, task, group_feat, vect_textual, vect_w2v):
    event = events[idx_event]

    location = os.path.dirname(os.path.realpath(__file__))
    train_event_file = event + "_task%s_sample_ground_truth" % (task) + ".csv"
    ...
    for k, (train, test) in enumerate(kf.split(X, y)):
        X_train, X_test, vect = get_train_test_features(
            X, train, test, features, vect_textual, vect_w2v
        )
