...
def get_train_test_features(X, train, test, features, vect_textual, vect_w2v):
    ...
    if "tweet.text" in features and len(features) > 1:
        ...
        X_train_text_w2v = map(lambda x: rich_analyzer_w2v(x), X_train["tweet.text"])
        ...
        X_test_text_w2v = map(lambda x: rich_analyzer_w2v(x), X_test["tweet.text"])
...
def adapt_features(df, dates):
    ...
    df["label_num"] = df["label"].apply(lambda x: int(1) if x == "yes" else int(0))
